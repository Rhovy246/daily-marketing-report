...
2. Topline summary as a small set of clearly labeled figures from Meta: total ad spend, total leads, and cost per lead.
...
6. New members (include ONLY if the JSON provides HubSpot "newMembers"): one plainly-worded line — "New members yesterday: N — M originally came from paid social ads" (use hubspot.newMembers.total and hubspot.newMembers.fromPaidSocial). Add a brief note that members who join now usually came from ads placed weeks earlier, so this reflects the ads' lifetime return, not same-day. Do not invent or recompute these numbers.
